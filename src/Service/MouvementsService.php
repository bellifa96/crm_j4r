...
class MouvementsService
{
    ...
    public function materail_hs_depot($data)
    {
        if ($data["depot"]  == 1) {
            return $this->materiel_HS_depot_layher($data);
        } else {
            return  $this->materiel_HS_depot_lagny($data);
        }
    }
    public function perte_stock($data)
    {
        if ($data["depot"]  == 1) {
            return $this->pertes_stock_layher($data);
        } else {
            return  $this->pertes_stock_lagny($data);
        }
    }
    public function vol_depot($data)
    {
        if ($data["depot"]  == 1) {
            return $this->vol_depot_layher($data);
        } else {
            return  $this->vol_depot_lagny($data);
        }
    }
    ...
    public function pertes_chantier($data){
        try {
            ...
            $mouvement
                ...
                ->setPrixloc($article->getPrixloc())
                ->setPrixvente($article->getPrixvente())
                ->setCodechantier($data["codeChantierInput"])
                ->setSens(9)
                ->setArticle($article->getArticle());
